...
public class AirbusDTO {
	...
	private Set<TratteDTO> tratte= new HashSet<TratteDTO>();
	...
	public Set<TratteDTO> getTratte() {
		return tratte;
	}

	public void setTratte(Set<TratteDTO> tratte) {
		this.tratte = tratte;
	}
	...
	public static AirbusDTO buildAirbusDTOFromModel(Airbus airbusModel, boolean includesTratte) {
		...
		if(includesTratte)
			result.setTratte(TratteDTO.createTrattaDTOSetFromModelSet(airbusModel.getTratte(), false));
		return result;
	}
	...
	public static List<AirbusDTO> createAirbusDTOListFromModelList(List<Airbus> modelListAirbus, boolean includesTratte) {
		return modelListAirbus.stream().map(airbusEntity->{
			...
			if (includesTratte) {
				result.setTratte(TratteDTO.createTrattaDTOSetFromModelSet(airbusEntity.getTratte(), false));
			}
			return result;
